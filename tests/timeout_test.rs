...
async fn test_process_cleanup_on_timeout() {
    ...
    assert!(
        ...
        "Operation took too long, process may not have been cleaned up properly"
    );
    ...
    assert!(
        duration2.as_secs_f64() < 5.0,
        "Second operation took too long, process may not have been cleaned up properly"
    );
